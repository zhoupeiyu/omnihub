  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (chunk) => {
      ...
      if (size > MAX_PDF_BYTES + 1024 * 1024) {
        reject({ status: 413, message: `PDF 不能超过 ${MAX_PDF_LABEL}` });
        req.destroy();
        return;
      }
      chunks.push(chunk);
...
export async function createPdfCompressionJob(req, res) {
  ...
  if (file.buffer.length > MAX_PDF_BYTES) throw { status: 413, message: `PDF 不能超过 ${MAX_PDF_LABEL}` };
