...
export const FEED_SOURCES = [
  {
    ...
    type: "aihot",
    api: "https://aihot.virxact.com/api/public/items?mode=all&take=80",
    rich: true,
...
/** 解析 AI HOT 富 API 响应为统一条目（保留精选分/分类/发布时间） */
...
export async function fetchFeed(sourceId) {
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: { "User-Agent": "OmniHub/1.0 (+feed reader)", "Accept": accept },
